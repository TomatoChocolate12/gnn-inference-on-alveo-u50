...
static void load_inputs(
    ...
    load_h_in: for (int i = 0; i < 2708 * 1433; ++i) {
...
#pragma HLS PIPELINE II=1
 w_stream << w[i];
    ...
    load_adj: for (int i = 0; i < 10556; ++i) {
#pragma HLS PIPELINE II=1
 adj_val_stream << adj_values[i];
        adj_col_stream << adj_col_indices[i];
    }
    load_row: for (int i = 0; i < 2708 + 1; ++i) {
#pragma HLS PIPELINE II=1
...
static void compute_gcn(
    ...
    static hls_dtype h_in_buf[2708][1433];
...
#pragma HLS ARRAY_PARTITION variable=w_buf complete dim=2
 ...
 static hls_dtype adj_val_buf[10556];
    static int adj_col_buf[10556];
    static int adj_row_buf[2708 + 1];


    read_h_in: for (int i = 0; i < 2708; ++i) {
...
#pragma HLS PIPELINE II=1
 w_buf[i][j] = w_stream.read();
    ...
    read_adj: for (int i = 0; i < 10556; ++i) {
#pragma HLS PIPELINE II=1
 adj_val_buf[i] = adj_val_stream.read();
        adj_col_buf[i] = adj_col_stream.read();
    }
    read_row: for (int i = 0; i < 2708 + 1; ++i) {
#pragma HLS PIPELINE II=1
 adj_row_buf[i] = adj_row_stream.read();
    }


    static hls_dtype aggregated_features[2708][1433];
...
#pragma HLS PIPELINE II=1
 VITIS_LOOP_97_3: for (int j = 0; j < 1433; ++j) {
    ...
    spmm_nodes: for (int i = 0; i < 2708; ++i) {
...
#pragma HLS UNROLL factor=UNROLL_FACTOR
 hls_dtype feat = h_in_buf[neighbor_idx][f];
    ...
    gemm_nodes: for (int i = 0; i < 2708; ++i) {
#pragma HLS PIPELINE II=1
...
static void store_result(
    ...
    VITIS_LOOP_156_1: for (int i = 0; i < 2708; ++i) {
...
#line 23 "/home/yashas.b/gnn-inference-on-alveo-u50/kernel/_x/gcn_layer_hls.hw/gnn/gnn.tcl"
#pragma HLSDIRECTIVE TOP name=gnn
# 172 "/home/yashas.b/gnn-inference-on-alveo-u50/kernel/gnn_hls.cpp"

#pragma HLS INTERFACE m_axi port = h_in offset = slave bundle = gmem0 depth=(2708 * 1433)
...
#pragma HLS INTERFACE m_axi port = adj_row_ptr offset = slave bundle = gmem3 depth=(2708 + 1)
#pragma HLS INTERFACE m_axi port = h_out offset = slave bundle = gmem0 depth=(2708 * 16)
